...
class VulkanApp : public Engine::GLFWCallbacks
{
public:
	...
	void Key(GLFWwindow* pWindow, int Key, int Scancode, int Action, int Mods)
	{
		...
		if (!Handled) {
			Handled = HandleCameraKeys(m_pGameCamera, Key, Action);
		}
	}
	...
	{
		HandleCameraMouseButton(m_pGameCamera, Button, Action);
	}
...
private:
	...
	void DefaultCreateCameraPers(float FOV, float zNear, float zFar)
	{
		...
		PersProjInfo persProjInfo = { FOV, (unsigned int)m_windowWidth, (unsigned int)m_windowHeight,
									  zNear, zFar };

		glm::vec3 Pos(0.0f, 0.0f, 0.0f);

		m_pGameCamera = new Camera();
		m_pGameCamera->Init(Pos, persProjInfo);
	}
	...
	Engine::VulkanCore m_vkCore;
	Engine::VulkanQueue* m_pQueue = NULL;
