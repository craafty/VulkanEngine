...
namespace Engine {
	...
	VulkanCore::~VulkanCore()
	{
		...
		if (!vkDestroySurface) {
			MY_ERROR("Cannot find address of vkDestroySurfaceKHR\n");
			exit(1);
		}
		...
		if (!vkDestroyDebugUtilsMessenger) {
			MY_ERROR("Cannot find address of vkDestroyDebugUtilsMessengerEXT\n");
			exit(1);
		}
	...
	void VulkanCore::CreateDebugCallback()
	{
		...
		if (!vkCreateDebugUtilsMessenger) {
			MY_ERROR("Cannot find address of vkCreateDebugUtilsMessenger\n");
			exit(1);
		}
	...
	void VulkanCore::CreateDevice()
	{
		...
		if (m_physDevices.Selected().m_features.geometryShader == VK_FALSE) {
			MY_ERROR("The Geometry Shader is not supported!\n");
		}

		if (m_physDevices.Selected().m_features.tessellationShader == VK_FALSE) {
			MY_ERROR("The Tessellation Shader is not supported!\n");
		}
